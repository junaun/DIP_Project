...
class LaTexParser extends Parser {
  ...
  void tokenize() {
    ...
    final sqrt =
        (string('\\sqrt') & char('{').and()).map((v) => ['\\sqrt', 'f']);
    final nrt = (string('\\sqrt') & char('[').and()).map((v) => ['\\nrt', 'f']);
    final simplefunction = ((string('\\sin') |
                string('\\cos') |
                string('\\tan') |
                string('\\arcsin') |
                string('\\arccos') |
        ...
        .map((v) => [v, 'f']);
    final otherfunction =
        (string('\\frac') | string('\\log')).map((v) => [v, 'f']);
    final function = simplefunction | otherfunction | sqrt | nrt;

    final lp = (string('\\left(') | char('{') | string('\\left|') | char('['))
        .map((v) => [v, 'l']);

    final rp = (string('\\right)') | char('}') | string('\\right|') | char(']'))
        .map((v) => [v, 'r']);
    ...
    final times = string('\\times').map((v) => [
          v,
          ['o', 3, 'l']
        ]);

    final divide = string('\\div').map((v) => [
          v,
          ['o', 3, 'l']
        ]);
    ...
    for (var i = 0; i < _stream.length; i++) {
      ...
      if (i > 0 &&
          ...
          _stream[i - 1][1] == 'l' &&
          _stream[i][0] == '-' &&
          _stream[i + 1][1].contains(RegExp(r'[bfl]'))) {
          ...
          while (true) {
            ...
            if (_operStack.last[1] is List) {
              if (_operStack.last[1][1] > _stream[i][1][1]) {
                ...
                _operStack.removeLast();
                continue;
              } else if (_operStack.last[1][1] == _stream[i][1][1] &&
                  _operStack.last[1][2] == 'l') {
                _outputStack.add(_operStack.last[0]);
                _operStack.removeLast();
                continue;
              }
...
class MatrixParser extends Parser {
  ...
  @override
  void tokenize() {
    final matrix = (string('\\begin{bmatrix}') &
            any().starLazy(string('\\end{bmatrix}')).flatten() &
            string('\\end{bmatrix}'))
        .pick(1)
        .map((v) => [v, 'b']);
